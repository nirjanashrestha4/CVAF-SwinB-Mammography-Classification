...
Strategy 4 — 3:1 Cyclic Oversampling.
Malignant training patients are cyclically repeated until the count reaches
floor(N_benign / 3), producing a 3:1 class ratio in the training split.
All benign patients are retained — no data is discarded.
...
def oversample_3to1(samples: List[dict], seed: int = 42) -> List[dict]:
    """Cyclically oversample malignant training patients to reach 3:1 ratio.

    All benign patients are kept (no data discarded).
    Target malignant count = floor(N_benign / 3).
    Malignant patients are repeated cyclically to reach the target.

    Formula:
        N_malignant_target = floor(N_benign / 3)
        oversampled = (malignant_list * ceil(target / N_malignant))[:target]
    """
    ...
    n_ben     = len(benign)
    n_mal_orig = len(malignant)
    n_mal_target = n_ben // 3  # floor division

    # Cyclic repeat then trim to target
    repeats = (n_mal_target // n_mal_orig) + 1
    mal_repeated = (malignant * repeats)[:n_mal_target]

    oversampled = benign + mal_repeated
    rng.shuffle(oversampled)

    oversample_factor = n_mal_target / n_mal_orig

    print(f'  [3:1 Oversample] Benign={n_ben}  '
          f'Malignant={n_mal_target} (from {n_mal_orig}, '
          f'factor={oversample_factor:.2f}x)  '
          f'Total={len(oversampled)}  Ratio=3:1')
    return oversampled
...
def build_dataloaders(
    ...
    """Build train, validation and test DataLoaders with 3:1 oversampling."""
    ...
    # ── Apply 3:1 oversampling to TRAINING split ONLY ────────────────────
    print('Applying 3:1 cyclic oversampling to malignant training patients ...')
    train_s = oversample_3to1(train_s, seed=seed)

    print('\n-- Final training split after 3:1 oversampling ------------------')
    b = sum(1 for x in train_s if x['label'] == 0)
    m = sum(1 for x in train_s if x['label'] == 1)
    print(f'  Train : {len(train_s):5d}  Benign={b}  Malignant={m}  Ratio=3:1')
