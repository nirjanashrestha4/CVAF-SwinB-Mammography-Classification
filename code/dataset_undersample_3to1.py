...
Strategy 3 — 3:1 Random Undersampling.
Benign training patients are randomly reduced to 3x the malignant count,
producing a 3:1 class ratio in the training split only.
...
def undersample_3to1(samples: List[dict], seed: int = 42) -> List[dict]:
    """Undersample benign training patients to 3x the malignant count (3:1).

    N_benign_kept = min(N_benign, 3 * N_malignant)
    ...
    n_mal     = len(malignant)
    n_ben_target = min(len(benign), 3 * n_mal)
    benign_sampled = rng.sample(benign, n_ben_target)

    balanced = benign_sampled + malignant
    rng.shuffle(balanced)

    print(f'  [3:1 Undersample] Benign={len(benign_sampled)}  '
          f'Malignant={n_mal}  Total={len(balanced)}  Ratio=3:1')
    print(f'  [3:1 Undersample] Discarded {len(benign) - n_ben_target} benign patients')
    return balanced
...
def build_dataloaders(
    ...
    """Build train, validation and test DataLoaders with 3:1 undersampling."""
    ...
    # ── Apply 3:1 undersampling to TRAINING split ONLY ───────────────────
    print('Applying 3:1 undersampling to training split ...')
    train_s = undersample_3to1(train_s, seed=seed)

    print('\n-- Final training split after 3:1 undersampling -----------------')
